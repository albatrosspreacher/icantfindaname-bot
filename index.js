...
const bot = new Telegraf(
    process.env.BOT_TOKEN); // get the token from envirenment variable
bot.start(
    ctx => ctx.reply(
        "Hello! Please use /help for more information.")); // display Welcome
                                                           // text when we start
                                                           // bot
bot.hears(
    "hi",
    ctx => ctx.reply(`Hi ${
        ctx.from
            .first_name}! For more information please go to /help :)`)); // listen
                                                                         // and
                                                                         // handle
                                                                         // when
                                                                         // user
...
bot.command("categories", ctx => {
  ctx.telegram.sendMessage(
      ctx.chat.id, "Choose from the options listed below!", {
        reply_markup : {
          inline_keyboard : [
...
bot.catch((err, ctx) => { console.log(`error ;-;\n ${err}`); });
bot.action("Capitalism", ctx => {
  ctx.deleteMessage();
  ctx.telegram.sendMessage(
      ctx.chat.id, "Here are some references for Capitalism!", {
        reply_markup : {
          inline_keyboard : [
            [ {
              text : "Capitalism 101",
...
bot.action("Communism", ctx => {
  ctx.deleteMessage();
  ctx.telegram.sendMessage(
      ctx.chat.id, "Here are some references for Communism!", {
        reply_markup : {
          inline_keyboard : [
            [ {
